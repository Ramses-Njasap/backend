...
class VerificationView(APIView):
    ...
    def check_verification_validity(self, check_on,
                                    request_data, user_instance) -> bool:

        if check_on == "phone":
            _otp_instance = _OTP(
                otp=request_data["otp"], user=user_instance,
                model=PhoneNumberVerificationOTP, database_actions=True)

        elif check_on == "email":
            _otp_instance = _OTP(
                otp=request_data["otp"], user=user_instance,
                model=EmailVerificationOTP, database_actions=True)
    ...
    def post(self, request):
        ...
        if user_exists:
            ...
            if verification_type and verification_type.lower() in {"phone", "email"}:
                is_valid = self.check_verification_validity(
                    check_on=verification_type.lower(),
