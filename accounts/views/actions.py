...
from accounts.views.user_actions.login import (Login as UserLogin,
                                               LoginWithCode as UserLoginWithCode,
                                               LoginWithoutDeviceToken)

from accounts.views.client_actions.refresh_access_token import (
    RefreshAccessToken as ClientRefreshAccessToken)

from accounts.views.user_actions.refresh_access_token import (
    RefreshAccessToken as UserRefreshAccessToken)
...
class RedirectLoginView(APIView):
    ...
    def post(self, request):
        ...
        #                           args=(request.device_meta_info,
        #                           user_ip, self.handle_geolocation,
        #                           user_instance))
...
class RequestLoginOTP(APIView):
    ...
    def post(self, request):
        ...
        if not user_instance:
            response.errors(
                field_error=("User Error: Couldn't Find User"
                             " With The Inputted Credentials."),
                for_developer=(f"""User Error: Couldn't Find User
                               With The Inputted Credentials ({email_or_phone})."""),
                code="NOT_FOUND",
                status_code=404
            )
        ...
        otp = OTPGenerator(
            ...
            model=LoginOTP, user=user_instance)
        ...
        send_verification_thread = threading.Thread(
            target=getattr(
                Verification(user=user_instance, model=LoginOTP),
                verification_type
            )
        )
...
class ChangePasswordAPIView(APIView):
    ...
    def password_similarity(self, current_password: str,
        ...
        similarity = SequenceMatcher(
            None, current_password, new_password).ratio()

        return similarity

    # Cannot use validate_password function name since
    # it is already an inbuilt serializer function name
    # following the serializer validate function naming
    # pattern validate_{field_name}
    ...
    def post(self, request):
        ...
        if None in [current_password, new_password]:
            response.errors(
                field_error=("Missing Field Error: Make Sure To"
                             " Fill In Both Current And New Password."),
                for_developer=("Missing Field Error: Make Sure To Fill"
        ...
        if request.user.check_password(current_password):
            ...
            similarity_score = self.password_similarity(
                current_password=current_password, new_password=new_password)

            if similarity_score < 0.6:
                response.errors(
                    field_error=("New And Current Passwords Are Similar."
                                 " Try With A Different Password."),
                    for_developer=(f"""Current And New Password Had A
                                   Similarity Ratio Of {similarity_score}
                                   Which Is Less Than 0.6."""),
                    code="NOT_ACCEPTABLE",
                    status_code=406
                )

            validated_password = self.validate_user_password(
                value=current_password, user_instance=request.user)
