...
class UserProfile(models.Model):
    ...
    # instead of an instance ([]) so that
    # it is not shared between all field instances
        ...
        validators=[
            MinLengthValidator(
                ...
                message="Phone number must be entered in the format: '+999999999'. "
                        "With a minimum of 8 digits allowed."
            ),
            RegexValidator(
                regex=r"^\+[0-9]{7,15}$",
                message="Phone number must be entered in the format: '+999999999'. "
                        "From 8 up to 16 digits allowed."
            ),
        ]
    )
    legal_name = models.CharField(max_length=50, null=True, blank=True)
    gender = models.CharField(
        max_length=8, choices=UserGender.choices, default=UserGender.OTHER)
