...
***MLM USER DEFINITION***
            A user who decides to become a multi level marketing
            user has the privilege to sell on behalf other
            users or companies thereby receiving commission
            from each sale they make.

            They also have the privilege to have commission from every
            new user in the pyramid system.

***THE PYRAMID SYSTEM***
            Multilevel marketing users can invite a maximum of two users.
            These new users (if they also decide to become multilevel marketing users)
            have the privilege to invite a maximum of two other users
            and so on and so forth thereby creating a Pyramid structure of users.

***THE PYRAMID STRUCTURE***
            As the Pyramid grows, the commission reduces till in reaches 0.
            When a user no longer gain subscription plan commission from
            his/her Pyramid system, that pyramid system ceases to grow
            thereby limiting the user to gain only from the existing pyramid system
            user based on sales of properties only.
...
class MLMUser(models.Model):
    ...
    def calculate_recruiter_commission(self, amount: Decimal = Decimal('0.00'),
                                       recruiter: User = None) -> Decimal:

        # Calculate the commission for the recruiter
        plan = recruiter.subscription_plan
        commission_percentage = plan.new_user_commission_percentage
    ...
    def recruit(self, new_user: User = None):
        ...
        # Set the new user's level to zero
        new_mlm_user = MLMUser(user=new_user, level=0)
        new_mlm_user.save()
        ...
        above_users_mlm = MLMUser.objects.filter(
            children=new_mlm_user).order_by('level')

        for above_user_mlm in above_users_mlm:
            commission = self.calculate_recruiter_commission(
                commission, above_user_mlm.user)

            above_user_mlm.balance += commission
            above_user_mlm.level += 1

            # Check if the earning is almost zero (rounded down)
            if above_user_mlm.balance.quantize(
                    Decimal('1.00'), rounding=ROUND_DOWN) == Decimal('100.00'):
                # Stop increasing levels and commissions for users earning close to zero
                break
...
class MLMRelationship(models.Model):
    parent = models.ForeignKey(
        MLMUser, on_delete=models.CASCADE,
...
class MLMConfig(models.Model):
    ...
    qualification_sales = models.DecimalField(
