...
class UserProfileManager(models.Manager):
    def filter(self, auto: bool = True,
    ...
    def filter_active_profiles(self, auto: bool = True,
                               *args, **kwargs) -> models.QuerySet:

        if not auto:
            return self.filter(auto=False, *args,
                               **kwargs, user__is_active=True)

        return self.filter(*args, **kwargs, user__is_active=True)

    def filter_inactive_profiles(self, auto: bool = True,
                                 *args, **kwargs) -> models.QuerySet:

        if not auto:
            return self.filter(auto=False, *args,
                               **kwargs, user__is_active=False)

        return self.filter(*args, **kwargs, user__is_active=False)

    def get(self, auto: bool = True,
        ...
        try:
            ...
            for_developer = (f"""Multiple {self.model.__name__}
                              Instances Match The Given Query Parameters.
                              Try Using .filter*** And You'll Get These Instances:
                              {str(filtered_data)}""")
    ...
    def get_active_profile(self, query_id: str = None,
                           auto: bool = True, *args, **kwargs) -> models.Model:
        ...
        if not auto:
            if query_id:
                return self.get(auto=False, user__query_id=query_id,
                                *args, **kwargs, user__is_active=True)
            else:
                return self.get(auto=False, *args,
                                **kwargs, user__is_active=True)

        else:
            if query_id:
                return self.get(user__query_id=query_id,
                                *args, **kwargs, user__is_active=True)

            else:
                return self.get(*args, **kwargs, user__is_active=True)

    def get_inactive_profile(self, auto: bool = True,
                             *args, **kwargs) -> models.Model:
