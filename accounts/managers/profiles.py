...
class UserProfileManager(models.Manager):
    ...
    def get_active_profile(self, query_id: str = None, auto: bool = True, *args, **kwargs) -> models.Model:
        ...
        if not auto:
            if query_id:
                return self.get(auto=False, user__query_id=query_id, *args, **kwargs, user__is_active=True)
            else:
                return self.get(auto=False, *args, **kwargs, user__is_active=True)
        
        else:
            if query_id:
                return self.get(user__query_id=query_id, *args, **kwargs, user__is_active=True)
            else:
