...
class Partition(models.Model):
    ...
    name = models.CharField(_("Partition Name"), max_length=14,
...
class RoomPartition(models.Model):
    partition = models.ForeignKey(Partition, null=False, blank=False, on_delete=models.CASCADE,
                                  help_text='Room Partition')
    number = models.SmallIntegerField(default=1)
    query_id = models.BinaryField(null=False, blank=False, max_length=10000, db_index=True)

    def __str__(self) -> str:
        return f"{self.number} {self.partition.name}" if self.number > 1 else f"{self.number} {self.partition.name}s"
...
class Room(models.Model):
    ...
    home_type = models.CharField(max_length=100, choices=HomeType.choices, default=HomeType.SINGLE_ROOM)
    number_of_rooms = models.SmallIntegerField(default=1)
    rooms_taken = models.SmallIntegerField(default=0, null=True, blank=True)
    ...
    def rooms_remaining(self) -> int:
        return 0 if (self.rooms_taken > self.number_of_rooms) else (self.number_of_rooms - self.rooms_taken)
...
class Home(models.Model):
    ...
    uploader = models.ForeignKey(Profile, null=False, blank=False, on_delete=models.CASCADE)
    ...
    geom = models.PointField(srid=4326,  # WGS84 link to read: https://en.wikipedia.org/wiki/World_Geodetic_System
                             geography=True, dim=3, spatial_index=True,
    ...
    distance_from_road = models.FloatField(db_index=True, null=True, blank=True) # Calculated from `geom` (coordinate) values to nearest main road .

    boundary = models.PolygonField(srid=4326)

    land_boundary = models.OneToOneField(Boundary, null=True, blank=True, on_delete=models.CASCADE)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.multi_rooms and (self.rooms.count() is not None):
            response.websocket_errors(
                g_name='',
                info='Activate The Option To Save Multiple Rooms On This Property.',
                for_developer='Multiple Rooms Cannot Be Saved On This Property. Activate The Option To Save Multiple Rooms On This Property.',
