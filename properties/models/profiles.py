...
class Profile(models.Model):
    ...
    user_type = models.CharField(
        ...
        default=UserType.INDIVIDUAL, db_index=True
    )
    ...
    def save(self, *args, **kwargs):
        # Ensure statuses defaults to `BUYER`
        # if profile has no status
        if not self.statuses:
            self.statuses = [Profile.Status.BUYER]

        # Check if profile is newly created by
        # inspecting if the db table column has an id
        is_new = self.pk is None

        # Deactivate the profile if the
        # class instance is newly created
