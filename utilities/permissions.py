...
class UserAuthPermission(BasePermission):
    def check_user_active_status(self, request):
        if not request.user.is_active:
            response.errors(
                field_error = "User Inactive - Can't Grant User Permission",
                for_developer = "User Inactivate - Can't Grant User Permission",
...
class DeviceAuthPermission(BasePermission):
    ...
    def get_user_from_request_data(self, request_data):
        if not "email_or_phone" in request_data:
            response.errors(
                            field_error = "Something Went Wrong In Authenticating This Device",
                            for_developer = "Device Authentication Failed: No Device Token And `email_or_phone` Attribute in `request body`",
                            code = "BAD_REQUEST",
                            status_code = 400
                        )
        ...
        if not user_instance:
            response.errors(
                        field_error = f"Failed To Authenticate User: User With Credential {email_or_phone} Does Not Exist.",
                        for_developer = f"Device Authentication Failed: No Device Token And No User With Credential {email_or_phone} Exist.",
                        code = "BAD_REQUEST",
                        status_code = 400
                    )
    ...
    def validate_device_token(self, request, access_token):
        ...
        login_url = settings.APPLICATION_SETTINGS["LOGIN_URL"]["BASE"] + settings.APPLICATION_SETTINGS["LOGIN_URL"]["URL"]
        if request.path.lstrip("/") != login_url:
            _login_history = self.check_login_history(device_instance=device_instance)
    ...
    def check_device_authentication_header(self, request):
        if "Device-Authorization" in request.headers:
            ...
            # No Need To Check Login History During Login
            login_url = settings.APPLICATION_SETTINGS["LOGIN_URL"]["BASE"] + settings.APPLICATION_SETTINGS["LOGIN_URL"]["URL"]
            request_login_otp_url = settings.APPLICATION_SETTINGS["REQUEST_LOGIN_OTP"]["BASE"] + settings.APPLICATION_SETTINGS["REQUEST_LOGIN_OTP"]["URL"]
            
            if (request.path.lstrip("/") == login_url) or (request.path.lstrip("/") == request_login_otp_url):
                if not request.data:
                    response.errors(
                        field_error = "Something Went Wrong In Authenticating This Device.",
                        for_developer = "Device Authentication Failed: No Device Token And `request body` Not Found",
                        code = "BAD_REQUEST",
                        status_code = 400
                    )
                
                user_instance = self.get_user_from_request_data(request_data=request.data)
                
                # Performing Actions To Verify User With New Device Attempting Login
                check_verified_credentials_instance = CheckVerifiedCredentials(user_instance=user_instance)

                if check_verified_credentials_instance is None:
                    response.errors(
                        field_error = "Something Went Wrong In Authenticating This Device.",
                        for_developer = "Device Authentication Failed: No Device Token And Not A Trusted Device To Perform Login With OTP.",
                        code = "BAD_REQUEST",
                        status_code = 400
                    )
                
                else:
                    phone, email, device = check_verified_credentials_instance.is_phone_email_verified(check_device=True)
                    ...
                    perm_response(res={"send_otp_to": send_otp_to}, status_code=307)
    ...
    def check_login_history(self, device_instance):
        ...
        if latest_login_history_instance:
            if (latest_login_history_instance.logout_at is not None) and (latest_login_history_instance.logout_at >= latest_login_history_instance.login_at):
                # Add Code To Revoke Device Token Cancelled.
                response.errors(
                    field_error="User Has Been Logged Out. Try To Login Again",
                    for_developer="User Has Been Logged Out. Redirect To Login Page",
...
class GrantPermission(DeviceAuthPermission, UserAuthPermission):
    ...
    def has_permission(self, request, view):
        """
        Check For Device Permission First. The Backend Has Trusts With The Client (Device)
        More Than It Has Trust With The User. This Is Because The Client Is The One Interacting
        With The Server And Not The User. Instead, The User Creates Trust With The Client And Not
        With The Server. So, Validation Of The Client (Device) Is More Important Than The User.
        """
        # Checking (Client) Device Permission
        device_permission = super(DeviceAuthPermission, self).has_permission(request, view)
        
        if not device_permission:
            return False

        # Checking (End) User Permission
        user_permission = super(UserAuthPermission, self).has_permission(request, view)
        
        if not user_permission:
            return False

        # Return True only if both user and device have permission
        return True