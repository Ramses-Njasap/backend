...
class UserAuthToken:
    ...
    def get_token_expiration_date(self, token):
        ...
        token_exp = datetime.datetime.fromtimestamp(
            token.payload["exp"], tz=datetime.timezone.utc
        )
    ...
    def refresh_access_token(self, refresh_token,
        ...
        try:
            ...
            token_exp = [
                self.get_token_expiration_date(token)
                for token in tokens
            ]
...
class DeviceAuthenticator:
    def __init__(
            self, secret_key=settings.DEVICE_JWT["SIGNING_KEY"],
            ...
            database_actions=True, user_instance: User = None,
    ...
    def perform_database_actions(
            self, access_token, refresh_token, access_token_expires_at,
        ...
        if not refresh_access:
            ...
            try:
                ...
                while DeviceToken.objects.filter(
                ...
                device_token_instance = DeviceToken.objects.create(
            ...
            try:
                device_token_instance = DeviceToken.objects.filter(
                    refresh_token=refresh_token
                ).last()
    ...
    def _for_old_device(
        ...
        try:
            ...
            if self.user:
                response.errors(
                    ...
                    for_developer=(
                        f"""Device With ID {self.instance.pk}
                         Was Not Found"""
                    ),
                    code="NOT_FOUND",
                    status_code=1011,
        ...
        expired_since = (
            datetime.datetime.now() - device_token_instance.refresh_token_expires_at
        ).days

        # Refresh access token delay
        rf_acc_delay = settings.APPLICATION_SETTINGS[
            "DEVICE_REFRESH_ACCESS_TOKEN_DELAY"
        ]

        if expired_since <= rf_acc_delay:
        ...
        if _is_database_actions_successful is None:
            old_device_instance.refresh_token_renewal_count = (
                device_instance.refresh_token_renewal_count
            )
            ...
            if self.user:
                response.errors(
                    field_error=(
                        "Unable To Renew Both Device Tokens"
                        " (access and refresh tokens)"
                    ),
                    for_developer=(
                        "An Error Occurred When Trying To Perform Database"
                        " Actions On The Already Generated Tokens"
                    ),
                    code="INTERNAL_SERVER_ERROR",
                    status_code=1011,
                    main_thread=False,
                ...
                response.errors(
                    ...
                    for_developer=(
                        "An Error Occurred When Trying To Perform"
                        " Database Actions On The Already Generated Tokens"
                    ),
                    code="INTERNAL_SERVER_ERROR",
                    status_code=500,
                    main_thread=True
    ...
    def generate_tokens(self, is_old_device: bool = False):
        ...
        # Combine device ID and current timestamp
        ...
        access_token = jwt.encode(
            access_payload, self.secret_key,
            algorithm=settings.DEVICE_JWT["ALGORITHM"]
        )

        refresh_token = jwt.encode(
            refresh_payload, self.secret_key,
            algorithm=settings.DEVICE_JWT["ALGORITHM"]
        )
        ...
        if _is_successful:
            access_token_data = [access_token, access_token_exp]
            refresh_token_data = [refresh_token, refresh_token_exp]
    ...
    def verify_access_token(self, access_token):
        try:
            ...
            payload = jwt.decode(
                ...
                algorithms=[settings.DEVICE_JWT["ALGORITHM"]]
            )
            return payload
    ...
    def blacklist_access_token(self, access_token):
        ...
        try:
            ...
            #     status_code=500
            # )
            # Not Seeing A Reason Why The Entire
            # Process Should Fail Because Token Failed To
            # Be Blacklisted
            pass
    ...
    def revoke_token(self, refresh_token):
        # In a real system, you might want to maintain
        # a list/database of revoked tokens
        # Here, we'll just print a message indicating
        # token revocation
    ...
    def refresh_access_token(self, refresh_token):
        try:
            ...
            expiration_time = datetime.datetime.utcfromtimestamp(refresh_payload['exp'])
            ...
            if current_time < expiration_time:
                ...
                new_access_token_payload = {
                    ...
                    'exp': (
                        ...
                        + datetime.timedelta(days=self.access_token_lifetime)
                    )
                }
                new_access_token = jwt.encode(
                    new_access_token_payload, self.secret_key,
                    algorithm=settings.DEVICE_JWT["ALGORITHM"]
                )
                ...
                new_refresh_token_payload = {
                    ...
                    'exp': (
                        datetime.datetime.utcnow()
                        + datetime.timedelta(days=self.refresh_token_lifetime)
                    )
                }
                new_refresh_token = jwt.encode(
                    new_refresh_token_payload, self.secret_key,
                    algorithm=settings.DEVICE_JWT["ALGORITHM"]
                )
                ...
                new_access_token = [
                    new_access_token, new_access_token_payload["exp"]
                ]
                new_refresh_token = [
                    new_refresh_token, new_refresh_token_payload["exp"]
                ]
                ...
                response.errors(
                    ...
                    for_developer=(
                        ...
                        " The Refresh Token Used Has Expired"
                    ),
    ...
    def decode_token(self, access_token: str,
                     get_device_instance: bool = False,
