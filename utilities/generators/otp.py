...
class OTPGenerator:
    def __init__(
            self, secret_key: str, model: models.Model,
            user: User = None, req: bool = False, save: bool = True
    ):
    ...
    def generate_otp(self):
        ...
        if not _otp_instance._check_model_validity():
            ...
            response.errors(
                ...
                for_developer=(f"""Model ({self.model._meta.model_name})
                                Not A Valid Model For This Operation. Valid
                                Models Are {dir(OTPModels)}"""),
        ...
        response.errors(
            ...
            for_developer=("Maximum Tries To Create Unique OTP Exceeded."
    ...
    def _is_otp_used(self, otp: str) -> tuple:
        ...
        # If OTP does not exist, it should create
        # a new OTP instance and save to database

        # return statements:
        # tuple[0] Does self.model instance otp exists? tuple[1]
        # is self.model instance otp active?

        otp_instance, is_otp_created = OTP.objects.get_or_create(otp=otp)

        model_instance, is_model_created = self.model.objects.get_or_create(
            user=self.user
        )
        ...
        if not is_model_created:
            ...
            if not is_otp_created:
                ...
                used_otp_instance = model_instance.used_otp.filter(
                    otp=otp_instance
                ).exists()
                ...
                if used_otp_instance:
                    ...
                    if used_otp_instance.first().is_active:
                        ...
                        # used otp instance for user in self.model
                        # exists and it is the active instance in self.model
                        return True, True

                    else:
                        # used otp instance for user in self.model
                        # exists and it is not active in self.model
                        return True, False

            else:
                # Used OTP instance for user in self.model
                # does not exists and cannot be active in self.model
                # since it doesn't exist in the first place
                return False, False

        else:
            # The OTP exists but does not exists in self.model
            # so all returns False
            return False, False

    def _mark_otp_used(self, otp, is_instance_active: bool = False):
        # Implement the logic to mark the OTP as used
        # This could involve storing the used OTP in
        # a database or another storage mechanism
            ...
            response.errors(
                ...
                for_developer=(
                    f"""Model ({self.model._meta.model_name})
                     Has No Relationship With {OTP._meta.model_name} or
                     Model ({self.model._meta.model_name}) Is Not A ForeignKey To
                     {OTP._meta.model_name}"""
                ),
                code="BAD_REQUEST",
                status_code=400
            )
        ...
        used_otp_instance = UsedOTP.objects.create(
            otp=otp_instance, is_active=True
        )
...
class OTPVerifier:
    ...
    def verify_otp(self, user_otp, user_id, window=1):
        # Set the maximum number of attempts to verify the
        # OTP within the window
        max_attempts = 10
    ...
    def _verify_current_otp(self, user_otp, user_id, window):
        ...
        # Calculating the counter based on the current
        # time and time step
        counter = current_time // time_step

        # Iterating over a time window to verify OTPs
        for i in range(window + 1):
            ...
            # Extracting the dynamic truncation offset
            # from the last 4 bits of the digest
            offset = hmac_digest[-1] & 0x0F

            # Extracting a 4-byte dynamic binary code from the digest
            binary_code = hmac_digest[offset:offset + 4]

            # Converting the binary code to an integer
            # and format to be 6 digits
            calculated_otp = int.from_bytes(binary_code, byteorder='big') % 10**6
            calculated_otp_str = f'{calculated_otp:06d}'

            # Check if the calculated OTP matches the
            # user-provided OTP
