...
class DeviceAuthenticator:
    def __init__(
            self, secret_key: str = settings,
            token_expiry_minutes=15, refresh_token_expiry_days=30
    ):
    ...
    def generate_tokens(self, device_id):
        ...
        access_payload = {
            ...
            'exp': (
                ...
                + datetime.timedelta(minutes=self.token_expiry_minutes)
            )
        }
        ...
        refresh_payload = {
            ...
            'exp': (
                datetime.datetime.utcnow()
                + datetime.timedelta(days=self.refresh_token_expiry_days)
            )
        }
    ...
    def revoke_refresh_token(self, refresh_token):
        ...
        # maintain a list/database of revoked tokens
    ...
    def refresh_access_token(self, refresh_token):
        try:
            ...
            refresh_token_timeout = _datetime.utcfromtimestamp(refresh_payload['exp'])

            # Check if the refresh token is still valid
            if _datetime.utcnow() < refresh_token_timeout:
                ...
                new_access_token = jwt.encode(
                    new_access_token_payload, self.secret_key, algorithm='HS256'
                )
                ...
                new_refresh_token = jwt.encode(
                    new_refresh_token_payload, self.secret_key, algorithm='HS256'
                )
...
class DeviceSignature:
    ...
    def to_database(self) -> bytes:
        """
        Most database table's column's unique identifies
        are mostly integers incrementally starting from zero(0).
        It will be more secure to generate an identifier that is
        1) Deficult to memorise since knowing table's column's
           unique identifier permits you to access all other
           information in that row
        2) One can be able to extract important information
           from the unique identifier if gotten (reasons why the
           encoding needs to be secured).
                    Reasons being that, if the client is unable to
                    access the database, the client with authorization
                    to the encoding algorithm will be able to extract
                    these important information without needing to access
                    the database. So, some clients will be given authority
