...
class Verification:
    ...
    def send_via_email(self, model_otp_instance: models.Model = None):
        ...
        if model_otp_instance is not None:
            ...
            try:
                send_email_task.delay(template_id=template_id, user_pk=self.user.pk, otp_code=otp_code, recipient_list=recipient_list)
    ...
    def send_code_to_user(self, model_instance: models.Model):
        ...
        if is_valid:
            ...
            if model_instance._meta.model_name == PhoneNumberVerificationOTP._meta.model_name:
                self.send_via_sms(model_otp_instance=model_otp_instance)

            elif model_instance._meta.model_name == EmailVerificationOTP._meta.model_name:
                self.send_via_email(model_otp_instance=model_otp_instance)
            
            elif model_instance._meta.model_name == LoginOTP._meta.model_name:
                self.send_via_email(model_otp_instance=model_otp_instance) if self.user.email else self.send_via_sms(model_otp_instance=model_otp_instance)
            

    def _check_generated_code_existence(self):
        try:
            # Please, note: model_otp_instance is the model instance belonging 
            # to a particular user. I'm unable to think of a suitable variable name for it
...
class CheckVerifiedCredentials:
    """
        This checks if the various user credentials has been verified. It follows the hierachal order
    ...
    def is_phone_email_verified(self, check_device=False, **kwargs):
        ...
        return ["phone", self.is_phone_Verified()], ["email", self.is_email_Verified()], device_check
    
    def is_email_Verified(self):
        try:
            account_verification_instance = AccountVerification.objects.get(user=self.user)
        except (AccountVerification.DoesNotExist, Exception):
            return False
        
        return True if account_verification_instance.email_verified else False
    ...
    def is_phone_Verified(self):
        ...
        return True if account_verification_instance.phone_number_verified else False
