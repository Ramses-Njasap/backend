...
class NameValidation:
    ...
    def validate_pattern(self) -> tuple:
        ...
        min_length = 6
        max_length = 15
        # Regex pattern for usernames (letters and digits only,
        # no spaces or special characters)
        # ^ asserts the start of the string
        # [a-zA-Z0-9] matches any letter
        # (uppercase or lowercase) or digit
        # {min_length,max_length} specifies the
    ...
    def validate(self) -> tuple:
        ...
        # Check if all functions in criterions
        # return True using all()
...
class EmailValidation:
    ...
    ----> Used in:
               1) User serializer
                 (accounts/serializers/users [UserSerializer])
    """
    ...
    def validate_uniqueness(self) -> tuple:
        if User.verify.email_exists(self.email):
            message = f"User With This Email ({self.email}) Already Exists"
            return False, "EMAIL_CONFLICT", 409, message

        message = f"This Email ({self.email}) Is Unique"
        return True, "EMAIL_ACCEPTED", 202, message

    def validate_pattern(self) -> tuple:
        # Regular expression pattern for a basic
        ...
        message = f"This Email Address ({self.email}) Is Valid"
        return True, "VALID_EMAIL", 202, message
    ...
    def validate(self) -> tuple:
        ...
        if self.uniqueness and self.pattern:
            ...
            if result_uniqueness[0] and result_pattern[0]:
                ...
                message = f"Acceptable Email Address ({self.email})"
                return True, "EMAIL_ACCEPTED", 202, message
        ...
        if not email_validation[0]:
            if email_validation[1] == 'EMAIL_CONFLICT':
                ...
                field_message = (
                    ...
                    " Example email: johndoe@email.com"
                )
...
class PhoneValidation:
    def __init__(
            self, phone: str, uniqueness: bool = True,
            pattern: bool = True
    ):
    ...
    def validate_pattern(self) -> tuple:
        # Regular expression pattern for a basic
        # phone number address validation
        ...
        if not phone_validation[0]:
            if phone_validation[1] == 'PHONE_CONFLICT':
                ...
                field_message = (
                    ...
                    " Example phone number: +123456789034765"
                )
                for_developer = (
                    "Invalid Phone Pattern."
                    " Example phone number: +123456789034765"
...
class PasswordValidation:
    def __init__(self, password: str,
                 user: dict = {}, similarity: bool = True,
                 strength: bool = True):
    ...
    def validate_similarity(self) -> tuple:
        ...
        if self.user == {}:
            # Check if the password contains any substrings
    ...
    def validate_strength(self) -> tuple:
        ...
        len_msg = "Password must be at least 12 characters long"
        uppercase_msg = "Password must contain at least one uppercase letter"
        lowercase_msg = "Password must contain at least one lowercase letter"
        digit_msg = "Password must contain at least one digit"
        sx_msg = "Password must contain at least one special character"
        criteria = [
            (len(self.password) >= 12, len_msg, 400),
            (re.search(r'[A-Z]', self.password), uppercase_msg, 400),
            (re.search(r'[a-z]', self.password), lowercase_msg, 400),
            (re.search(r'\d', self.password), digit_msg, 400),
            (re.search(r'[!@#$%^&*()_+{}\":<>?]', self.password), sx_msg, 400)
        ]

        failed_criteria = [
            (message, status_code)
            for condition, message, status_code in criteria
            if not condition
        ]
    ...
    def validate_or_raise(self):
        ...
        if not password_validation[0]:
            if password_validation[1] == "PASSWORD_BAD_REQUEST":
                ...
                field_message = (
                    "Password Must Contain Atleast: 12 Characters,"
                    " 1 Uppercase and Lowercase,"
                    " 1 Digit and"
                    " 1 Special Character And Must Not Be Similar To"
                    " Any Of Your User's Information"
                )
